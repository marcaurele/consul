...
func (c *cmd) Run(args []string) int {
	...
	if c.listPrimaryKeys {
		...
		for _, response := range responses {
			c.UI.Output("")
			c.UI.Output(poolName(response.Datacenter, response.WAN, response.Segment) + ":")
			c.UI.Output(formatMessages(response.Messages))
			c.UI.Output(formatKeys(response.PrimaryKeys, response.NumNodes))
		}
		return 0
	}
...
func poolName(dc string, wan bool, segment string) string {
	...
	return fmt.Sprintf("%s (%s)%s", dc, pool, segment)
}
